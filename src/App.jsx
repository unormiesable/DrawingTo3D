...
if (stroke.path.length < 2) return; context.strokeStyle=stroke.color; context.lineWidth=stroke.brushSize;
  context.beginPath(); context.moveTo(stroke.path[0].x, stroke.path[0].y); for (let i=1; i < stroke.path.length; i++) {
  context.lineTo(stroke.path[i].x, stroke.path[i].y); } context.stroke(); }); }, [canvasRef, pathsRef]); useEffect(()=>
  {
  setupCanvas();
  }, [setupCanvas]);
  ...
  const svg = `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
    <circle cx="${size / 2}" cy="${size / 2}" r="${(size / 2) - 1}" stroke="black" stroke-width="1" fill="none" />
  </svg>`;
  const cursorUrl = `url('data:image/svg+xml;base64,${btoa(svg)}') ${size / 2} ${size / 2}, auto`;
  setCursorStyle(cursorUrl);
  } else if (mode === 'erasing') {
  const eraseSize = 30;
  const svg = `<svg width="${eraseSize}" height="${eraseSize}" xmlns="http://www.w3.org/2000/svg">
    <circle cx="${eraseSize / 2}" cy="${eraseSize / 2}" r="${(eraseSize / 2) - 1}" stroke="black" stroke-width="1"
      fill="none" /></svg>`;
  ...
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
    <h1>Gambar Ikan</h1>
    <canvas style={{ border: '2px solid black', borderRadius: '8px', cursor: cursorStyle }} ref={canvasRef}
      onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseLeave={()=> (mode === 'drawing' ?
      finishDrawing() : null)}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', width: '80%', marginTop: '1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <input type="color" id="strokeColor" value={strokeColor} onChange={(e)=> setStrokeColor(e.target.value)}
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <input type="color" id="fillColor" value={fillColor} onChange={(e)=> setFillColor(e.target.value)}
          style={{ marginRight: '1rem' }}
          />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', flexGrow: 1, marginLeft: '2rem' }}>
          ...
          <input type="range" id="brushSize" min="1" max="50" value={brushSize} onChange={(e)=>
      ...
      <div style={{ display: 'flex', width: '80%', justifyContent: 'space-between', marginTop: '0.5rem' }}>
        <button style={{
            ...buttonBaseStyle,
      ...
      <div style={{ display: 'flex', width: '80%', justifyContent: 'center', marginTop: '0.5rem' }}>
        <button style={{
            ...buttonBaseStyle,
